...
# PRIVATE METHODS
_get_ec2_instance_id() {
    $JQ -r '.Instances[0].InstanceId' $1
}

_get_ec2_instance_public_ip(){
    INSTANCE_IP=""
    while [ -z $INSTANCE_IP ]
    do
...
_get_ec2_instance_private_ip(){
    $JQ -r '.Instances[0].PrivateIpAddress' $1
}

_create_ec2_resources() {
...
deployer_get_eip() {
    INSTANCE_ID=`_get_ec2_instance_id $WORKDIR/$INSTANCE_DESCRIPTION`
    EIP=`_get_ec2_instance_public_ip $INSTANCE_ID`
    echo $EIP
}

deployer_get_iip() {
    IIP=`_get_ec2_instance_private_ip $WORKDIR/$INSTANCE_DESCRIPTION`
    echo $IIP
}
